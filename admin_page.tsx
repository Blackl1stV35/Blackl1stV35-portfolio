...
const FIELDS: Record<CollectionName, FieldDef[]> = {
  ...
  activity: [
    { key: 'title',       label: 'Title *',             type: 'text' },
    { key: 'type',        label: 'Type *',              type: 'select', options: ['talk','workshop','competition','volunteer','conference','hackathon','event'] },
    { key: 'status',      label: 'Status *',            type: 'select', options: ['green','yellow','red'] },
    { key: 'organiser',   label: 'Organiser',           type: 'text' },
    { key: 'role',        label: 'Your role',           type: 'select', options: ['speaker','participant','organiser','mentor','judge','volunteer'] },
    { key: 'location',    label: 'Location',            type: 'text' },
  ...
  achievement: [
    { key: 'title',          label: 'Title *',           type: 'text' },
    { key: 'type',           label: 'Type *',            type: 'select', options: ['award','certification','scholarship','honour','ranking','grant'] },
    { key: 'status',         label: 'Status *',          type: 'select', options: ['green','yellow','red'] },
...
const SIDEBAR: { label: string; panel: Panel }[] = [
  { label: 'Profile',      panel: '_author'      },
  { label: 'Contact',      panel: '_contact'     },
  { label: 'Projects',     panel: 'projects'     },
  { label: 'Work',         panel: 'work'         },
  { label: 'Publications', panel: 'publications' },
  { label: 'Activity',     panel: 'activity'     },
  { label: 'Achievement',  panel: 'achievement'  },
  { label: 'Books',        panel: 'books'        },
  { label: 'CV & PDFs',    panel: '_uploads'     },
]
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (s) { setSession(s); setAuthed(true); loadAuthor(); loadContact() }
  }, [])
  ...
  async function loadAuthor() {
    try {
      ...
      if (res.ok) {
        ...
        setAuthorData({ ...rest, tags: Array.isArray(tags) ? tags.join(', ') : (tags ?? '') })
  ...
  async function handleAuth() {
    ...
    if (res.ok) {
      ...
      setSession(json.session); setAuthed(true); loadAuthor(); loadContact()
    } else if (res.status === 429) {
      setLocked(true); setError('Too many attempts. Locked for 15 minutes.')
    } else {
  ...
  async function saveAuthor() {
    ...
    const payload = {
      ...authorData, photo: authorPhoto ?? null,
    ...
    if (res.ok) { setAuthorSaved(true); setTimeout(() => setAuthorSaved(false), 3000) }
  ...
  async function saveContact() {
    ...
    if (res.ok) { setContactSaved(true); setTimeout(() => setContactSaved(false), 3000) }
    else { const j = await res.json(); alert('Error: ' + j.error) }
  }

  async function saveEntry() {
    setSaving(true)
    const fields = { ...formData, date: formData.date || new Date().toISOString().split('T')[0] }
    ...
    setSaving(false)
    if (res.ok) { setSaved(true); setTimeout(() => { setSaved(false); setShowForm(false); setFormData({}) }, 2500) }
    else { const j = await res.json(); alert('Error: ' + j.error) }
  }
  ...
  function renderField(f: FieldDef, data: Record<string, string>, set: (k: string, v: string) => void) {
    if (f.type === 'image') return (
      <div className="col-span-2 flex items-center gap-4">
        ...
        <span className="text-xs font-mono text-zinc-400 leading-relaxed">Optional · click to upload · stored in frontmatter</span>
      </div>
    )
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <div className="flex min-h-[calc(100vh-48px)]">
        <aside className="w-44 bg-white border-r border-zinc-100 py-4 flex-shrink-0">
          <div className="text-xs font-mono uppercase tracking-widest text-zinc-400 px-4 mb-2">Manage</div>
          {SIDEBAR.map(({ label, panel: p }) => (
            <button key={p} onClick={() => { setPanel(p); setShowForm(false) }}
              className={`w-full text-left text-sm font-serif px-4 py-2 border-l-2 transition-colors
                ${panel === p ? 'border-zinc-900 bg-zinc-50 text-zinc-900' : 'border-transparent text-zinc-500 hover:text-zinc-900'}`}
            >{label}</button>
          ))}
        </aside>
        ...
        <main className="flex-1 p-6 overflow-auto">
            ...
            <div>
              ...
              <div className="bg-white border border-zinc-200 rounded-md p-5 mb-4">
                ...
                <div className="flex items-center gap-5">
                  <AvatarUpload src={authorPhoto ?? undefined}
                    initials={authorData.name?.split(' ').map(w => w[0]).join('').slice(0,2) ?? 'AB'}
                    size={100} onChange={url => setAuthorPhoto(url)} />
                  <div className="text-xs font-mono text-zinc-400 leading-relaxed">Click to upload · saved to author.json via GitHub API<br />Displayed on About page + PDF cover · Max 5 MB</div>
                </div>
              </div>
              <div className="bg-white border border-zinc-200 rounded-md p-5 mb-4">
          ...
          {panel === '_contact' && (
            <div>
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-serif font-bold text-base">Contact details</h2>
                {contactSaved && <span className="text-xs font-mono text-green-600 flex items-center gap-1"><CheckCircle size={12} /> Saved &amp; committed</span>}
              </div>
              <div className="bg-white border border-zinc-200 rounded-md p-5 mb-4">
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    { key: 'formAction', label: 'Formspree URL *' },
                  ].map(f => (
                    <div key={f.key}>
                      <label className="block text-xs font-mono text-zinc-500 mb-1">{f.label}</label>
                      <input type="text" value={contactData[f.key] ?? ''}
                        onChange={e => setContactData(d => ({ ...d, [f.key]: e.target.value }))}
                        className="w-full border border-zinc-200 rounded px-2 py-1.5 text-sm font-serif focus:outline-none focus:border-zinc-400"
                      />
                    </div>
                  ))}
          ...
          {isCollection && (
            <div>
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-serif font-bold text-base capitalize">{panel}</h2>
                ...
                <div key={type} className="mb-4">
                  ...
                  >
                    ...
                    <div className="text-xs font-mono text-zinc-400 mt-1">
                      {type === 'cv' ? 'Replaces public/cv.pdf · served as /cv.pdf' : 'Vercel Blob · instant CDN URL'}
                    </div>
                  </div>
