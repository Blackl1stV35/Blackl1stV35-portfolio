...
const workLabels:        Partial<Record<StatusColor, string>> = { green: 'Current',   yellow: 'Part-time',    red: 'Closed'     }
const pubLabels:         Partial<Record<StatusColor, string>> = { green: 'Published',  yellow: 'Under Review', red: 'Retracted'  }
const bookLabels:        Partial<Record<StatusColor, string>> = { green: 'Read',       yellow: 'Reading',      red: 'Interested' }

interface Props {
  status: StatusColor
  variant?: 'default' | 'work' | 'publication' | 'book'
  /** Override the displayed label entirely — used by activity and achievement pages */
  customLabel?: string
  className?: string
}

export default function StatusBadge({ status, variant = 'default', customLabel, className = '' }: Props) {
