...
import {
  ...
  AlignmentType, BorderStyle, Footer, PageNumber, NumberFormat,
} from 'docx'
...
function hr(): Paragraph {
  return new Paragraph({ border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: LINE, space: 1 } }, spacing: { after: 120 }, children: [] })
}
    ...
    children.push(
      new Paragraph({
        children: [
          ...
          mono('   '),
          new TextRun({ text: ` ${e.status} `, size: 16, bold: true, color: statusColor(e.status), font: 'Courier New' }),
        ],
...
function activitySection(entries: ActivityEntry[], footer: Footer) {
  ...
  for (const e of entries) {
    children.push(
      new Paragraph({
        children: [run(String(e.title ?? ''), { bold: true, size: 24 })],
        spacing: { before: 200, after: 40 },
      }),
      new Paragraph({
        children: [
...
export async function buildPortfolioDOCX(): Promise<Buffer> {
  const author: Record<string, unknown> = await (async () => {
    try { return await readJSON(path.join(process.cwd(), 'content', 'author.json')) }
    catch { return JSON.parse(fs.readFileSync(path.join(process.cwd(), 'content', 'author.json'), 'utf-8')) }
  })()
