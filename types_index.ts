...
export interface ActivityEntry extends BaseEntry {
  type: 'talk' | 'workshop' | 'competition' | 'volunteer' | 'conference' | 'hackathon' | 'event'
  organiser?: string
  role?: 'speaker' | 'participant' | 'organiser' | 'mentor' | 'judge' | 'volunteer'
  location?: string
  url?: string
}

export interface AchievementEntry extends BaseEntry {
  type: 'award' | 'certification' | 'scholarship' | 'honour' | 'ranking' | 'grant'
  issuer?: string
  expiry?: string
